...
# Каждая орбита представляет из себя кортеж из пары чисел - полуосей ее эллипса. Площадь эллипса 
# вычисляется по формуле S = pi*a*b, где a и b - длины полуосей эллипса. При решении задачи
# используйте списочные выражения. Подсказка: проще всего будет найти эллипс в два шага: 
...
#     for k in orbits:
#         if k[0] != k[1]:
#             elips_orbit.append(k)
#     print(max(elips_orbit, key=lambda orb: 3.14 * orb[0] * orb[1]))

# find_farthest_orbit(orbits)

orbits = [(1, 3), (2.5, 10), (7, 2), (6, 6), (4, 3)]
def find_farthest_orbit(orbits):
    print(max([k for k in orbits if k[0] != k[1]], key=lambda orb: 3.14 * orb[0] * orb[1]))
